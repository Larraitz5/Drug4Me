...
from scripts.insert_data.insert_ddi import insert_ddi
from scripts.insert_data.insert_ema import insert_ema
from scripts.insert_data.insert_cima_esp import insert_cima_esp
from scripts.insert_data.insert_panel_esp import insert_paneldata_db
from scripts.insert_data.pharm_gkb.insert_drugs_gkb import insert_drugs_gkb
from scripts.insert_data.pharm_gkb.insert_drugslabel import insert_drugslabels
from scripts.insert_data.pharm_gkb.insert_relationships import insert_relationships
from scripts.insert_data.pharm_gkb.insert_clinicalAnnotations import insert_clinicalann_db
from scripts.insert_data.pharm_gkb.insert_phenoannotations import insert_phenotannotations_db
from scripts.insert_data.pharm_gkb.insert_variantannotations import insert_variantannotations_db
...
def get_next_version():
    ...
    db_path_ = properties["database_path"]
    db_list = [file for file in os.listdir(db_path_) if os.path.splitext(file)[-1] == ".db"]
    if len(db_list) == 0:
        return str(0)
    else:
...
def create_new_db_name():
    """Crea un nuevo nombre de archivo para la base de datos usando la fecha actual y la siguente versión disponible."""
...
# Crear engine para la base de datos y la base declarativa para definir las tablas
db_path = os.path.join(properties["database_path"], create_new_db_name())
...
class DrugsGkb(Base):
    ...
    def __init__(self, id_drug, name_gkb, generic_name_gkb, trade_name_gkb, external_voc_gkb,
                 rxnorm_identifiers_gkb,
                 atc_identifiers_gkb):
...
class VariantAnnotations(Base):
    ...
    def __init__(self, variant_annotation_id, variant_haplotypes, genes_varann, drugs_varann,
                 phenotype_category_varann,
...
def insert_data(session_):
    insert_cima_esp(session_, Drugscima)
    insert_paneldata_db(session_, PanelGeneticoEsp)
    insert_ema(session_, EmaTable)
    insert_drugs_gkb(session_, DrugsGkb)
    insert_drugslabels(session_, DrugsLabelGkb)
    insert_relationships(session_, RelationshipsGkb)
    insert_clinicalann_db(session_, ClinicalAnnGkb, ClinicalAnnAlleles)
    insert_variantannotations_db(session_, VariantAnnotations)
    insert_phenotannotations_db(session_, PhenoAnnotations)
    insert_ddi(session_, DrugDrugInteractions)


# Crear las tablas en la bd
Base.metadata.create_all(engine)

# Insertar los datos en la bd, para ello se crea la sesion
